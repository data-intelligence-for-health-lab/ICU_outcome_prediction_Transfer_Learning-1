...
# function for classification model definition
def my_model(layer = 128, dropout_rate = 0.5,lr=0.001,activation='relu',kernel_initializer='he_uniform', kernel_regularizer=tf.keras.regularizers.l2(l2=1e-3)):
    tf.keras.backend.clear_session()

    model = tf.keras.models.Sequential()

    model.add(tf.keras.layers.Dense(layer, activation=activation, kernel_initializer=kernel_initializer,kernel_regularizer=kernel_regularizer))
    model.add(tf.keras.layers.Dropout(dropout_rate))

    model.add(tf.keras.layers.Dense(layer/2, activation=activation, kernel_initializer=kernel_initializer,kernel_regularizer=kernel_regularizer))
    model.add(tf.keras.layers.Dropout(dropout_rate))
    
    model.add(tf.keras.layers.Dense(layer/4, activation=activation, kernel_initializer=kernel_initializer,kernel_regularizer=kernel_regularizer))
    model.add(tf.keras.layers.Dropout(dropout_rate))
...
# function for regression model definition
def my_model_REG(layer = 128, dropout_rate = 0.5,lr=0.001,activation='relu',kernel_initializer='he_uniform', kernel_regularizer=tf.keras.regularizers.l2(l2=1e-3)):
    tf.keras.backend.clear_session()
    ...
    model.add(tf.keras.layers.Dense(2*layer, activation=activation, kernel_initializer=kernel_initializer,kernel_regularizer=kernel_regularizer))
    model.add(tf.keras.layers.Dropout(dropout_rate))
...
X_train_df_original = pd.read_pickle('Data/MIMIC_master_X_train.pickle', compression = 'zip')
...
y_train_df_original = pd.read_pickle('Data/MIMIC_master_y_train.pickle', compression = 'zip')
...
for random_state in random_states:
    ...
    _, X_train_df, _, y_train_df = train_test_split(X_train_df_original, y_train_df_original, test_size=0.25, random_state = random_state)    
...
# # 1. 30_DAYS_EXPIRE processing started
    ...
    # hyper parameter values from the corresponding transfer learning model - MIMIC_30_DAYS_EXPIRE_R20_SM_929.h5
    layer,dropout_rate,lr,activation,kernel_initializer,kernel_regularizer = 128, 0.3, 0.001, 'relu', 'HeUniform', tf.keras.regularizers.l1(l1=0.001)
    batch_size = 128
    
    # model definition and building model
    model = my_model(layer,dropout_rate,lr,activation,kernel_initializer,kernel_regularizer)
    input_shape = (None, 104)
    model.build(input_shape)

    # fitting model
    model.fit(X_r, y_r,epochs = 50, \
      verbose = 0, callbacks= [early_stop_fcnn],batch_size = batch_size,validation_data=(X_val, y_val))   
    ...
    r_states = [random.randint(1,10000) for x in range(100)]
    ...
    for i in range(N):
        ...
        y_pred = (model.predict(X_test_boot) > 0.5).astype("int32").flatten()
    ...
    Boot_stats.to_csv('Results/MIMIC_30_DAYS_EXPIRE_FCNN_Bootstrap_25p_'+str(random_state)+'.csv')
...
# # 2. AKI
    ...
    # hyper parameter values from the corresponding transfer learning model - eCrit_AKI_R20_SM_1708.h5
    layer,dropout_rate,lr,activation,kernel_initializer,kernel_regularizer = 256, 0.3, 0.0001, 'PReLU', 'HeNormal', tf.keras.regularizers.l1(l1=0.001)
    batch_size = 32
    ...
    # saving bootstrap results for each random state and for each outcome
    Boot_stats.to_csv('Results/MIMIC_AKI_FCNN_Bootstrap_25p_'+str(random_state)+'.csv')
...
# # 3. H_LOS processing started
    ...
    y_train = y_train_df[y_train_df.H_LOS < np.percentile(y_train_df.H_LOS,98)]
    # selecting X_train rows for the corresponsing y_train rows using SUBJECT_ID
    X_train = X_train_df.loc[X_train_df['SUBJECT_ID'].isin(y_train['SUBJECT_ID'])]
    # dropping SUBJECT_ID column
    X_train = X_train.drop(columns = ['SUBJECT_ID'])
    ...
    y_val = y_val_df[y_val_df.H_LOS < np.percentile(y_val_df.H_LOS,98)]
    X_val = X_val_df.loc[X_val_df['SUBJECT_ID'].isin(y_val['SUBJECT_ID'])]
    X_val = X_val.drop(columns = ['SUBJECT_ID'])
    y_val = y_val['H_LOS']    
    
    y_test = y_test_df[y_test_df.H_LOS < np.percentile(y_test_df.H_LOS,98)]
    X_test = X_test_df.loc[X_test_df['SUBJECT_ID'].isin(y_test['SUBJECT_ID'])]
    X_test = X_test.drop(columns = ['SUBJECT_ID'])
    y_test = y_test['H_LOS']

    # hyper parameter values from the corresponding transfer learning model - eCrit_H_LOS_R20_LM_827.h5
    layer,dropout_rate,lr,activation,kernel_initializer,kernel_regularizer = 256, 0.4, 0.001, 'PReLU', 'HeNormal', tf.keras.regularizers.l2(l2=1e-3)
    batch_size = 64
 
    # model definition and building model 
    model = my_model_REG(layer,dropout_rate,lr,activation,kernel_initializer,kernel_regularizer)
    input_shape = (None, 104)
    model.build(input_shape)
    ...
    # saving bootstrap results for each random state and for each outcome
    Boot_stats.to_csv('Results/MIMIC_H_LOS_FCNN_Bootstrap_25p_'+str(random_state)+'.csv')
...
# # 4. ICU_LOS
    ...
    # hyper parameter values from the corresponding transfer learning model - eCrit_ICU_LOS_R20_LM_772.h5
    layer,dropout_rate,lr,activation,kernel_initializer,kernel_regularizer = 256, 0.3, 0.0001, 'LeakyReLU', 'HeNormal', tf.keras.regularizers.l2(l2=1e-3)
    batch_size = 32
    ...
    # saving bootstrap results for each random state and for each outcome    
    Boot_stats.to_csv('Results/MIMIC_ICU_LOS_FCNN_Bootstrap_25p_'+str(random_state)+'.csv')
